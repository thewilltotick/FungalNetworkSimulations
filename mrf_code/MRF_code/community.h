...
class Community {
 public:
  ...
  // copy graph
  Community (Graph g, int nb_pass, double min_modularity, double t);
  ...
  map<int,double> neigh_comm(int node);
...
inline double
Community::modularity_gain(int node, int comm, double dnodecomm) {
  assert(node>=0 && node<size);
