...
Community::modularity() {
  ...
  for (int i=0 ; i<size ; i++) {
    if (tot[i]>0)
      q += (double)in[i]/m2 - time*((double)tot[i]/m2)*((double)tot[i]/m2);
  }
...
Community::neigh_comm(int node) {
  map<int,double> res;
  pair<int *,double *> p = g.neighbors(node);
  ...
  for (int i=0 ; i<deg ; i++) {
    ...
    if (neigh!=node) {
      map<int,double>::iterator it = res.find(neigh_comm);
      if (it!=res.end())
...
Community::partition2graph() {
  ...
  for (int i=0 ; i<size ; i++) {
    pair<int *,double *> p = g.neighbors(i);
...
Community::partition2graph_binary() {
  ...
  g2.weights  = (double *)malloc((long)10000000*8);
  ...
  for (int comm=0 ; comm<comm_deg ; comm++) {
    ...
    for (int node=0 ; node<comm_size ; node++) {
      pair<int *,double *> p = g.neighbors(comm_nodes[comm][node]);
...
//      if (node%1000000==0) {fprintf(stderr,"%d ",node); fflush(stderr);}
      ...
      for (map<int,double>::iterator it=ncomm.begin() ; it!=ncomm.end() ; it++) {
