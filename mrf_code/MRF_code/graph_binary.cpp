...
Graph::Graph(char *filename, int type) {
  ...
  // IF WEIGHTED : read weights: 16 bytes for each link (each link is counted twice)
  if (type==WEIGHTED) {
    weights = (double *)malloc((long)nb_links*16);
    finput.read((char *)weights, (long)nb_links*16);
    total_weight=0;
    for (int i = 0 ; i<nb_links*2 ; i++) {
      //cerr << "weight = " << weights[i] << ", ";
...
Graph::display() {
  for (int node=0 ; node<nb_nodes ; node++) {
    pair<int *,double *> p = neighbors(node);
